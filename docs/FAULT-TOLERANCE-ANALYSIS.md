...
| **FT-1** | ~~Persistent Raft log via L3Store~~ — **Storage impl done (2026-04-12)**. New `aeon_cluster::log_store::L3RaftLogStore` (behind `cluster` feature) implements `RaftLogStorage` + `RaftLogReader` over `Arc<dyn L3Store>`. Key layout: `raft/vote`, `raft/committed`, `raft/last_purged`, `raft/log/{be_u64:index}` (big-endian so `scan_prefix` yields entries in log order). Hot metadata (vote, committed, last_purged, last_log_id) cached in `Arc<RwLock<_>>` so `get_log_state` is O(1); caches hydrated from L3 on `open()`. `append` / `truncate` / `purge` use `write_batch` for atomic multi-key updates (purge writes the last-purged marker in the same batch as entry deletions). 9 unit tests (roundtrip, reopen-survives-restart, truncate tail, purge head, vote/committed persistence, big-endian key ordering invariant) pass. **Follow-up**: wire into `ClusterNode::new_*` constructors behind a `cluster.raft_log.persist: true` config knob so production clusters can opt in; `MemLogStore` remains the default for single-node dev. | GAP 1, L3Store adapter pattern | Storage done / wiring pending | FT-9 |
...
| **TR-3** | Connection backoff with jitter across all connectors — **In progress**. ✅ Shared `BackoffPolicy { initial_ms, max_ms, multiplier, jitter_pct }` + stateful `Backoff` iterator added to `aeon-types` (defaults 100 ms → 30 s cap, ×2, ±20% jitter; 11 tests). ✅ Wired into **MQTT source** (resets on successful Publish or ConnAck). ✅ Wired into **MQTT sink** (eventloop poller resets on successful poll). ✅ Wired into **MongoDB CDC source** (resets on successful ChangeStreamEvent). ✅ Wired into **cluster `QuicEndpoint::connect_with_backoff()`** — opt-in retry variant for bootstrap/join flows; leaves plain `connect()` fail-fast for the openraft RPC path (openraft has its own protocol-level retry and blocking a client task there would delay heartbeats). **Follow-ups (deferred):** introduce reconnect loops in Postgres CDC / MySQL CDC / RabbitMQ / NATS source (currently single-shot — errors propagate up; adding reconnect is a feature, not a backoff-swap); HTTP polling source error path; WebSocket/Redis Streams once they land; WebTransport T3 SDK layer. | GAP H | Medium |
