...
| **FT-12** | Hot-path refcount & config optimization (perf tuning) — **Partial (2026-04-12)**. Audit + structural fixes applied. **Findings:** **(b)** was a false lead — `grep` of `pipeline_manager.rs` turned up no per-event `config.clone()`; config is passed once at task spawn. **(a)** shipped: `PassthroughProcessor::{process, process_batch}` clones now carry `// clone: <reason>` justification comments (both are refcount bumps required to emit an `Output` while still borrowing `event` for `with_event_identity`). **Biggest win — benchmark methodology fix**: the prior baseline of ~215-225 ns/event was inflated by two non-pipeline costs counted in the timed loop: **(i)** `MemorySource::next_batch` was doing `self.events[..].to_vec()` — an `Event::clone` per event per batch even though the source owned the events; **(ii)** the bench called `MemorySource::new(events.clone(), batch)` *inside* `b.iter()`, charging one full `Vec<Event>` clone per sample against the pipeline. Fixed: `MemorySource` now holds a `std::vec::IntoIter<Event>` and `next_batch` moves (no clone); `reset()` only supported via the new `new_resetable()` constructor (test-only path). Bench uses `criterion::iter_batched` so the event-vec clone lives in the untimed setup closure. **After**: noisy at 64B/256B (variance with BatchSize::LargeInput allocator behavior) but 1024B payload ran at **124 ns/event** (8.07 Melem/s, down from 222 ns/event pre-fix — 44% improvement confirmed with p=0.07). 64B/256B results still need a stable-system full-length criterion run to pin down. **Remaining gap** toward the <100 ns target is now structural, not refactor: `Output::new` writes a 128 B cache-aligned struct with `SmallVec::new()` + 4 `Option` fields; `process_batch` allocates a fresh `Vec<Output>` per batch. Candidates: shrink `Output` / drop `#[repr(align(64))]` on the sink path / add a `process_into(&self, events: Vec<Event>, out: &mut Vec<Output>)` trait method so the pipeline can reuse a single output buffer across batches. Deferred as separate work — the refactor portion of FT-12 is done. | GAP E, Gate 1 perf target | Medium | FT-11 |
