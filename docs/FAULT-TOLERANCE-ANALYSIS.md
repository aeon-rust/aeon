...
| **FT-12** | Hot-path refcount & config optimization (perf tuning) — **not a rule violation, but a Gate 1 latency optimization**. Refcount clones (`Bytes::clone`, `Arc::clone`, `Event::clone`) are cheap (~15-30ns each) but compound across pipeline stages. Two parts: **(a)** replace `Arc::clone`/`Bytes::clone`/`Event::clone` with `&Event`/`&Bytes` references where fan-out isn't required; add `// clone: <justification>` comments to legitimate clones. **(b)** move per-event `config.clone()` in `pipeline_manager.rs` to `Arc<Config>` so it's one refcount bump, not a struct deep-copy. Benchmark before/after with `gate1_steady_state` (target: <100ns per-event overhead). **Baseline captured 2026-04-12** via `blackhole_bench` (MemorySource → PassthroughProcessor → BlackholeSink, pure Aeon path): **~225 ns per event** (4.35–4.48 Melem/s), payload-size agnostic (64B/256B/1024B all within 2%). Gate 1 target <100 ns → **2.25× gap remaining**. Kafka-path bench (`gate1_steady_state` with BLACKHOLE_SINK, 20 K/s, 256 B): P50 2.5 ms, P99 25 ms, CPU 23 % — P99 tail is dominated by rdkafka consumer poll cadence, **not** Aeon compute (huge headroom). Structural cost centres identified: `PassthroughProcessor::process_batch` allocates `Vec<Output>` + per-event `Output::new` (128 B cache-aligned, SmallVec init, 4-Option `with_event_identity`) + Bytes/Arc refcount bumps. Reaching 100 ns likely requires reducing `Output` size or offering a zero-copy `&Event`-returning processor variant — structural change, not a refactor. | GAP E, Gate 1 perf target | Medium | FT-11 |
