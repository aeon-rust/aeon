...
| **G9.d.c.2** | **Per-pod processor / connector build — replace the warn-stubs.** ⏳ pending. Lifts `instantiate_processor` from `rest_api.rs` into the supervisor; the 5 stub methods (`cluster_blue_green_start`, `cluster_canary_start`, `cluster_reconfigure_source/sink`, `upgrade_running`) drop their `tracing::info!` deferral and actually build + drive the local `PipelineControl`. Composes with G9.d.c.1's drain so the swap fires after the per-partition boundary is reached. Live cluster validation on a 3-pod RD cluster: per-partition transitions at the named seq on every node, observable via `/poh-head` + per-pipeline metrics. | Closes the Layer-4 substrate end-to-end — followers no longer rely on the leader's REST handler for runtime convergence; sets up the Layer 5 (`WatermarkView` façade) work | `crates/aeon-engine/src/pipeline_supervisor.rs`, `crates/aeon-engine/src/rest_api.rs` (lift instantiate_processor + connector factories) |
