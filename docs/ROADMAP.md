...
| **G9.c** | **Sequence-bounded processor lifecycle (Path 2 = Layer 4 of the integrated stateful-processing trajectory).** Promoted 2026-05-03 from "deferred follow-up to G9.b" to **explicit v0.1 work** per [`docs/STATEFUL-PROCESSING-EVOLUTION.md`](STATEFUL-PROCESSING-EVOLUTION.md) § 4 Phase 1. Closes the cluster-correctness gap left by G9.b for the 7 vulnerable endpoints (`upgrade/blue-green`, `upgrade/canary`, `cutover`, `rollback`, `promote`, `reconfigure/source`, `reconfigure/sink`) AND establishes per-partition Raft-committed sequence boundaries as the substrate for Layer 5 (`WatermarkView` façade) and Layer 6 (windowing F1+F2). Adds 6 new `RegistryCommand` variants carrying `BTreeMap<PartitionId, Sequence>` boundary maps; new `PipelineControl::drain_partitions_at_seq()` reusing existing `WriteGate` + `EngineCutoverCoordinator::CutoverOffsets` + `L2BodyStore::iter_from(seq)` + `LivePohChainRegistry`. Backfills the existing `UpgradePipeline` to also propagate drain-and-swap to followers (closes the pre-existing pipeline-wide-only gap surfaced by the audit). Estimated effort: 4–5 days. **Inserted before Session B** so the EKS ceiling number reflects what operators would actually deploy in v0.1. ⏳ open. | Cluster correctness for full pipeline-lifecycle ops (today only `apply` / `start` / `stop` / `delete` are cluster-wide); foundational substrate for the deferred Layer 5+6 stateful-processing work | `crates/aeon-engine/src/pipeline.rs` (new `drain_partitions_at_seq`), `crates/aeon-engine/src/pipeline_supervisor.rs` (6 new lifecycle methods), `crates/aeon-engine/src/cluster_applier.rs` (dispatch new variants), `crates/aeon-types/src/registry.rs` (6 new `RegistryCommand` variants), `crates/aeon-engine/src/rest_api.rs` (refactor 7 handlers to `propose_registry`) |
