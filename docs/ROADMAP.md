...
| **G9.d.b** | **`PipelineControl::drain_partitions_at_seq` + leader-side `compute_partition_boundaries` helper.** ⏳ pending. Walks a `PartitionBoundaries` map, awaits per-partition `LivePohChainRegistry.sequence() >= target_seq`, then freezes the existing `WriteGate`. Free helper reads PoH heads at REST-handler time pre-propose. Unit tests on the drain primitive. | Strong-consistency boundary primitive — composes with G9.d.a's wire shape | `crates/aeon-engine/src/pipeline.rs`, `crates/aeon-engine/src/cluster_boundaries.rs` (new) |
