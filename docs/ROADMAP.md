...
| **G9.d.c** | **Per-pod processor / connector instantiation in `cluster_applier`.** ⏳ pending. Replaces the 5 supervisor stubs (`cluster_blue_green_start`, `cluster_canary_start`, `cluster_reconfigure_source/sink`, `upgrade_running`) with actual per-node build + swap, gated on the boundary map from G9.d.a. Currently leader-only via `state.pipeline_controls`. | Closes the Layer-4 substrate gap so followers run the runtime swap on their own pod, not just the leader's | `crates/aeon-engine/src/pipeline_supervisor.rs`, `crates/aeon-engine/src/cluster_applier.rs` |
