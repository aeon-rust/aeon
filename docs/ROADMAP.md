...
Committed this snapshot to stop-and-think before picking the next direction. Pillars 1/2/4/5/6 are complete. Pillar 3 has all transport primitives shipped (CL-6a bulk sync, CL-6b PoH transfer, CL-6c cutover handshake, CL-6d throttle + metrics) and as of 2026-04-23 the engine-side provider wiring (CL-6c.4) is closed too — G2 leader-side driver ships with it, leaving only CL-1 DOKS/EKS acceptance re-runs as open work in this pillar. Pillar 7 is blocked by design (library-ecosystem maturity in other languages).
...
| **G9** | Auto-forward (or 307-redirect) cluster-mutation writes to the Raft leader from any REST endpoint. ✅ shipped 2026-04-19 — `rest_api::maybe_forward_to_leader(state, uri)` helper returns `307 Temporary Redirect` with `Location: {scheme}://{leader_host}:{rest_port}{path+query}` plus `X-Leader-Node-Id` and `X-Aeon-Forwarded: 1` headers when the receiving pod is not the current Raft leader. Wired at the top of every cluster-mutation handler (`transfer_partition`, `cluster_drain`, `cluster_rebalance`, `register_processor`, `create_pipeline`, `start_pipeline`, `stop_pipeline`, `upgrade_pipeline`, `delete_pipeline`). Leader host comes from `membership.get_node(leader_id)` on the Raft metrics; REST port is the new `ClusterConfig::rest_api_port` (populated in `aeon-cli` from `AEON_API_ADDR` / CLI bind flag) and defaults to `http` scheme. Scripted clients that don't follow redirects can still read `X-Leader-Node-Id` to reroute. **Superseded 2026-05-03 by G9.b** — see next row for the silent-success bug the 307 path introduced. | Operator UX + harness simplification | Engine side — add leader-proxy or return a consistent 307 with `X-Leader-*` headers |
| **G9.c** | **Sequence-bounded processor lifecycle, declarative half (Layer 4 partial — split from full Layer 4 work).** ✅ **closed 2026-05-03** — closes the cluster-correctness gap left by G9.b for the 7 vulnerable endpoints (`upgrade/blue-green`, `upgrade/canary`, `cutover`, `rollback`, `promote`, `reconfigure/source`, `reconfigure/sink`) by Raft-replicating their declarative state to every node. Adds 7 new `RegistryCommand` variants (`BlueGreenStart`, `BlueGreenCutover`, `RollbackUpgrade`, `CanaryStart`, `CanaryPromote`, `ReconfigureSource`, `ReconfigureSink`) — each routed through `PipelineManager::apply()` (now dispatches the new variants to the existing direct methods); 7 new supervisor methods (`cluster_blue_green_start/cutover`, `cluster_rollback_upgrade`, `cluster_canary_start`, `cluster_promote_canary`, `cluster_reconfigure_source/sink`) that drive local PipelineControl on every node (cutover/rollback fully drive the runtime swap on whichever pod owns the local control handle; the instantiation-needing ops — blue-green-start, canary-start, reconfigure — are declarative-only on followers today, leader still does the runtime instantiation via `pipeline_controls`); cluster_applier dispatch for new variants + backfilled `UpgradePipeline` to also trigger `supervisor.upgrade_running` on every node; 7 REST handlers refactored to `propose_registry`. 7 new serde round-trip tests on `RegistryCommand` variants pin the wire format. Workspace: 1717 tests pass (was 1710), clippy + fmt clean. Live cluster validation on `aeon:v69`: `POST /api/v1/pipelines/g9c-test/cutover` against follower aeon-1 returned the leader's actual error message (`config error: pipeline 'g9c-test' is not in a blue-green upgrade`) — proves the request reached the leader through the cluster_write_forwarder middleware AND the leader proposed via Raft AND the leader's apply produced the meaningful error. Pre-G9.c the follower would have done a silent local mutation with no replication. **Per-partition sequence-bounded transitions** (the original Path 2 spec — boundary maps in variants, `PipelineControl::drain_partitions_at_seq()`, leader-side boundary computation from `LivePohChainRegistry`) are split into a follow-up atom **G9.d** so the silent-success bug ships fast in v0.1 and the deeper Layer-4 substrate work composes on top in v0.2. | Cluster correctness for full pipeline-lifecycle ops; G9.d follow-up adds the strong-consistency-at-boundary semantics that Layer 5+6 stateful-processing work needs | `crates/aeon-types/src/registry.rs` (7 new variants + 7 serde tests), `crates/aeon-engine/src/pipeline_manager.rs` (apply() dispatch), `crates/aeon-engine/src/pipeline_supervisor.rs` (7 cluster_* methods + `upgrade_running`), `crates/aeon-engine/src/cluster_applier.rs` (dispatch all 7 + UpgradePipeline backfill), `crates/aeon-engine/src/rest_api.rs` (7 handlers refactored to propose_registry) |
| **G9.d** | **Sequence-bounded transitions for processor lifecycle (full Layer 4 / Path 2 substrate).** Splits out from G9.c. Adds per-partition Raft-committed sequence boundary maps to the new variants + `PipelineControl::drain_partitions_at_seq()` reusing `WriteGate` + `EngineCutoverCoordinator::CutoverOffsets` + `L2BodyStore::iter_from(seq)` + `LivePohChainRegistry`. Per-pod processor / connector instantiation in cluster_applier (currently leader-only via `state.pipeline_controls` for instantiation-needing ops). Foundation for Layer 5 (`WatermarkView` façade) and Layer 6 (windowing F1+F2) per [`docs/STATEFUL-PROCESSING-EVOLUTION.md`](STATEFUL-PROCESSING-EVOLUTION.md) § 4 Phase 1. Estimated effort: 3–4 days. **Re-scoped 2026-05-06 into three sub-atoms** to ship in disciplined increments: **G9.d.a** (boundary-map schema, ⏳→✅ this commit), **G9.d.b** (drain primitive + leader-side compute helper, ⏳ next), **G9.d.c** (per-pod processor / connector instantiation in `cluster_applier`, ⏳ follow-up). Sequenced after G9.c (closed 2026-05-03) and before Layer 5 façade work. | Strong consistency at per-partition transition boundaries — partition X transitions at exactly seq N regardless of which pod owns it; substrate for windowing trigger semantics | `crates/aeon-engine/src/pipeline.rs` (new `drain_partitions_at_seq`), `crates/aeon-engine/src/pipeline_supervisor.rs` (per-pod processor instantiation), `crates/aeon-types/src/registry.rs` (extend G9.c variants with boundary maps), `crates/aeon-engine/src/cluster_applier.rs` (call instantiation-aware supervisor methods on every node) |
| **G9.d.a** | **Boundary-map schema on the 7 G9.c variants.** ✅ closed 2026-05-06 — extends `BlueGreenStart`, `BlueGreenCutover`, `RollbackUpgrade`, `CanaryStart`, `CanaryPromote`, `ReconfigureSource`, `ReconfigureSink` with a `boundaries: PartitionBoundaries` field (`PartitionBoundaries = BTreeMap<PartitionId, u64>`). `#[serde(default, skip_serializing_if = "BTreeMap::is_empty", with = "boundaries_serde")]` so v0.1 log entries (G9.c shape, no `boundaries` key) replay into the v0.2 shape with an empty map and re-serialise byte-identically — log replay across the in-place rolling-upgrade boundary stays correct. Custom `boundaries_serde` helper module stringifies the `u16` for JSON map keys (the JSON object-key constraint rejects native `u16` newtype keys; bincode wire format unaffected on the value type). `PipelineManager::apply` and the 7 REST handler call sites updated to the new variant shape (apply ignores `boundaries` — declarative state only; consumption lands in G9.d.b/c). 3 new aeon-types tests (`g9c_blue_green_start_replays_into_v2_with_empty_boundaries`, `g9c_lifecycle_variants_all_replay_with_empty_boundaries`, `g9d_populated_boundaries_round_trip`) plus the 7 existing G9.c roundtrips refreshed to assert empty-map skip-on-wire. **Workspace:** 59 test suites green, 0 failures; clippy `--all-targets -- -D warnings` clean; fmt clean. | Wire-format atom — locks the schema before downstream work consumes it; backward compat with v0.1 Raft logs verified at the unit-test level | `crates/aeon-types/src/registry.rs` (PartitionBoundaries type + boundaries_serde module + 7 variant fields + 3 backward/forward-compat tests), `crates/aeon-engine/src/pipeline_manager.rs` (apply() ignores boundaries), `crates/aeon-engine/src/rest_api.rs` (7 propose sites pass empty boundaries) |
| **G9.d.b** | **`drain_partitions_at_seq` + leader-side `compute_partition_boundaries` helper.** ✅ closed 2026-05-06 — new `aeon_engine::cluster_boundaries` module (gated `cluster` + `processor-auth`) wraps the existing `LivePohChainRegistry` (PoH chain state) and `WriteGateRegistry` (per-partition source freeze) primitives. `drain_partitions_at_seq(pipeline, boundaries, poh, gates, wait_timeout)` iterates the `PartitionBoundaries` map: per partition, polls `chain.sequence() >= target_seq` (2ms cadence, bounded by `wait_timeout`, default 5s exposed as `DEFAULT_DRAIN_WAIT`), then drives `WriteGate::request_freeze_and_drain`. Skip semantics: missing live PoH chain on this node (partition not running here) → silent skip with `tracing::debug!`, no gate freeze; missing `WriteGate` after waiting → skip freeze (no source task to drain); chain that doesn't reach target inside the budget → `AeonError::timeout` and gate left in `Open` (partial cutover state would be worse than no cutover). Empty `boundaries` is a no-op everywhere — preserves G9.c immediate-on-apply for v0.1 log entries. `compute_partition_boundaries(pipeline, partitions, poh)` is the leader-side companion: samples each partition's current PoH head sequence, omits partitions without a registered chain. **Tests:** 8 unit tests covering empty-map no-op, already-at-target fast path, wait-then-freeze with concurrent appender, timeout-leaves-gate-open, missing-chain skip, multi-partition end-to-end, leader-side compute on present-chains-only, leader-side compute on empty-partitions slice. **Workspace:** 59 test suites green, 0 failures; clippy `--all-targets -- -D warnings` clean; fmt clean. NB: implemented as free functions in `cluster_boundaries` rather than methods on `PipelineControl` (which has no registry handles in scope) — composability beats artificial enclosure. | Strong-consistency boundary primitive — composes with G9.d.a's wire shape; consumed by G9.d.c | `crates/aeon-engine/src/cluster_boundaries.rs` (new, 8 unit tests), `crates/aeon-engine/src/lib.rs` (module declaration gated on `cluster` + `processor-auth`) |
| **G9.d.c** | **Per-pod processor / connector instantiation in `cluster_applier`.** Replaces the 5 supervisor stubs (`cluster_blue_green_start`, `cluster_canary_start`, `cluster_reconfigure_source/sink`, `upgrade_running`) with actual per-node build + swap, gated on the boundary map from G9.d.a. **Re-scoped 2026-05-06 into two passes:** **G9.d.c.1** (boundary plumbing — drain primitive invoked from supervisor, ✅ this commit), **G9.d.c.2** (per-pod processor / connector build — replaces the warn-stubs with real instantiation, ⏳ follow-up). | Closes the Layer-4 substrate gap so followers run the runtime swap on their own pod, not just the leader's | `crates/aeon-engine/src/pipeline_supervisor.rs`, `crates/aeon-engine/src/cluster_applier.rs` |
| **G9.d.c.1** | **Boundary plumbing — `drain_partitions_at_seq` invoked from every cluster_* supervisor method.** ✅ closed 2026-05-06 — the 7 G9.c lifecycle supervisor methods (`cluster_blue_green_start`, `cluster_blue_green_cutover`, `cluster_rollback_upgrade`, `cluster_canary_start`, `cluster_promote_canary`, `cluster_reconfigure_source/sink`) plus the G9.c.4 `upgrade_running` backfill all gain a `boundaries: &PartitionBoundaries` arg. New private helper `PipelineSupervisor::drain_at_boundaries` calls `cluster_boundaries::drain_partitions_at_seq(name, boundaries, &poh_live_chains, gate_registry, DEFAULT_DRAIN_WAIT)` before each side-effect — the supervisor already owns both registries, so no new wiring on the construction path. `cluster_applier` extracts `boundaries` from each Raft-committed variant (clone before `pipelines.apply` moves the variant) and threads it through. Empty map (G9.c default + v0.1 log replay) yields a no-op drain; populated map awaits per-partition `chain.sequence() >= target` then freezes the per-partition `WriteGate` exactly as G9.d.b's primitive specifies. The 5 instantiation-needing stubs still warn-and-noop on the runtime side-effect (per-pod processor build is G9.d.c.2). UpgradePipeline calls pass an empty `PartitionBoundaries::new()` because that variant doesn't yet carry boundaries on the wire. The `drain_at_boundaries` helper compiles to a no-op on builds without `cluster + processor-auth` (the registries don't exist). **Workspace:** 59 test suites green, 0 failures; clippy `--all-targets -- -D warnings` clean; fmt clean; both feature paths (`--features cluster,processor-auth` and default) build. | Wires the G9.d.b primitive into the apply path so the boundary semantics are honoured even when the runtime swap stays leader-pinned in G9.d.c.2 | `crates/aeon-engine/src/pipeline_supervisor.rs` (drain_at_boundaries helper, 7 method signatures grow boundaries arg), `crates/aeon-engine/src/cluster_applier.rs` (extract + thread boundaries through every variant dispatch) |
| **G9.d.c.2** | **Per-pod processor / connector build — replace the warn-stubs.** **Re-scoped 2026-05-06 into two passes:** **G9.d.c.2.a** (processor build, ✅ this commit) — lifts `instantiate_processor` from `rest_api.rs` into the supervisor and replaces the 3 processor-instantiation stubs with real per-pod build + drive. **G9.d.c.2.b** (connector reconfigure, ⏳ follow-up) — `cluster_reconfigure_source/sink` need a type-erased `drain_and_swap_source_dyn` on `PipelineControl` because today's `drain_and_swap_source<S>` is generic over the concrete source type. Live cluster validation deferred to once c.2.b lands so the matrix can exercise both flavours. | Closes the Layer-4 substrate end-to-end — followers no longer rely on the leader's REST handler for runtime convergence; sets up the Layer 5 (`WatermarkView` façade) work | `crates/aeon-engine/src/pipeline_supervisor.rs`, `crates/aeon-engine/src/rest_api.rs` (lift instantiate_processor + connector factories) |
| **G9.d.c.2.a** | **Lift `instantiate_processor` into supervisor + wire 3 processor-instantiation methods.** ✅ closed 2026-05-06 — new `processor_registry: OnceLock<Arc<ProcessorRegistry>>` slot on `PipelineSupervisor` + `set_processor_registry` install + async `build_processor(name, version)` helper that mirrors `rest_api::instantiate_processor` but operates on the engine-internal handle. The 3 stub methods now do real per-pod work after `drain_at_boundaries`: `cluster_blue_green_start(name, &ProcessorRef, &boundaries)` builds the green processor and calls `PipelineControl::start_blue_green`; `cluster_canary_start(name, &ProcessorRef, initial_pct, &boundaries)` builds the canary and calls `start_canary` with the first step's traffic %; `upgrade_running(name, ProcessorRef, &boundaries)` builds the new processor and calls `drain_and_swap`. When the registry isn't installed (tests / benches), all three log a `warn` and skip the runtime swap — declarative state still replicates correctly. `cluster_applier` threads `new_processor` (and `steps.first().unwrap_or(10)` for canary) through. `aeon-cli::cmd_serve` calls `set_processor_registry(Arc::clone(&registry))` after the supervisor is constructed. T3/T4 (WebTransport / WebSocket) processor types surface a clear `AeonError::Config` ("hot-swap not supported") — same shape as the REST handler. **Connector reconfigure (`cluster_reconfigure_source/sink`) deferred to G9.d.c.2.b** because `PipelineControl::drain_and_swap_source<S>` is generic over the concrete source type and the supervisor only sees a type-erased factory result. **Workspace:** 59 test suites green, 0 failures; clippy `--all-targets -- -D warnings` clean; fmt clean. | Followers now build + drive the runtime swap on their own pod for blue-green / canary / upgrade — closes the leader-only convergence gap for these three lifecycle ops | `crates/aeon-engine/src/pipeline_supervisor.rs` (processor_registry slot, set_processor_registry install, build_processor helper, 3 method bodies replaced), `crates/aeon-engine/src/cluster_applier.rs` (extract + thread new_processor + steps), `crates/aeon-cli/src/main.rs` (set_processor_registry install in cmd_serve) |
| **G9.d.c.2.b** | **Type-erased connector hot-swap — close `cluster_reconfigure_source/sink` end-to-end.** ⏳ pending. Adds `drain_and_swap_source_dyn(Box<dyn Source>)` and `drain_and_swap_sink_dyn(Box<dyn Sink>)` to `PipelineControl` (today's `<S>`/`<K>` generics require the same concrete type as start; reconfigure with a different connector type can't use them). Supervisor's `cluster_reconfigure_source/sink` build the new connector via the existing `ConnectorRegistry` and drive the type-erased swap. Live cluster validation: 3-pod RD cluster, per-partition transitions at the named seq on every node, observable via `/poh-head` + per-pipeline metrics. | Closes the Layer-4 substrate end-to-end — supports cross-type source/sink reconfigure on every node; unblocks the Layer 5 (`WatermarkView` façade) work | `crates/aeon-engine/src/pipeline.rs` (PipelineControl type-erased swap methods + corresponding hot-swap path in run_buffered_managed), `crates/aeon-engine/src/pipeline_supervisor.rs` (replace 2 connector stubs) |
| **G9.b** | **Internal HTTP forwarder for cluster-write REST endpoints + comprehensive endpoint audit.** ✅ closed 2026-05-03 — surfaced during the v67 multi-node smoke: `aeon apply -f manifest.yaml --api <follower-url>` returned exit 0 with `pipeline 'foo' — created` and `Manifest applied.`, but the pipeline never landed in the cluster state. **Root cause:** the G9 307-redirect path is RFC-correct but `ureq` (the `aeon-cli` HTTP client) does **not** follow 3xx responses on POST per RFC 7231 §6.4.7 — the CLI saw the 307 as `Ok(_)` and printed success without ever reaching the leader. The same shape would break curl-without-`-L` and most off-the-shelf SDK clients. **Audit findings** (Explore agent, `crates/aeon-engine/src/rest_api.rs`): 9 endpoints used `maybe_forward_to_leader` and were silently broken (CREATE pipeline / processor, START / STOP / DELETE pipeline, upgrade, transfer_partition, cluster_drain, cluster_rebalance); 8 additional endpoints had no cluster awareness AT ALL and silently no-op'd on followers (`DELETE /processors/{name}/versions/{ver}`, `POST /pipelines/{name}/upgrade/{blue-green,canary}`, `cutover`, `rollback`, `promote`, `reconfigure/{source,sink}`); 2 endpoints are local-only by design and explicitly excluded (`pipelines/{name}/delivery/retry` per-node ledger, `processors/{name}/identities` per-node identity store). **Fix:** new `cluster_write_forwarder` axum middleware (`rest_api.rs`) intercepts every cluster-write request via the `is_cluster_write_path(method, path)` matcher (11 unit tests pinning the inventory), buffers the body, and proxies the entire HTTP request to the current leader's REST endpoint via `forward_to_leader_http` — returning the leader's response verbatim. Loop protection via `X-Aeon-Forwarded: 1` header → second hop returns HTTP 421 Misdirected Request rather than re-forwarding. Pass-through for GET/HEAD, local-only POSTs, and on the leader itself / standalone mode (no `cluster_node`). Why HTTP forwarding instead of QUIC `ProposeForward` for everything: the QUIC path only carries Raft proposes, but 8 of the vulnerable endpoints mutate `PipelineManager` / processor-registry state with no `RegistryCommand` variant yet (deferred to a follow-up Raft-replication atom). HTTP works uniformly today. **Cost:** one extra in-cluster HTTP round-trip on follower writes (sub-ms for co-located pods, single-digit ms cross-host). **Live cluster validation** on `aeon:v68`: the original reproducer (`aeon apply` against follower aeon-0) now correctly lands the pipeline in cluster state, observable via `aeon-2` (leader). The `maybe_forward_to_leader` 307 helper is kept as `#[allow(dead_code)]` for opt-in clients that explicitly want a redirect (future `X-Aeon-Accept-Redirects: 1` header gate). | Cluster correctness — operator can `kubectl exec` into any pod for cluster-write ops without secret-handshake leader discovery | `crates/aeon-engine/src/rest_api.rs` — new `cluster_write_forwarder` middleware, `forward_to_leader_http` helper, `is_cluster_write_path` matcher (+ 11 unit tests). Old per-handler `maybe_forward_to_leader` calls become dead code on the leader path (always return None when self is leader) and unreachable on followers (middleware fires first); marked deprecated, scheduled for removal in a follow-up atom along with the 8 vulnerable endpoints' Raft-replication variants. |
...
| V2 | T0–T6 matrix — **partial on RD**: T0 baseline, T2 3→5 STS-scale (exercises G10 seed-join code path; no node-pool resize on RD), T3 5→3→1 drain (G5 + G14 relinquish), T4 manual cutover (G11.a/b/c + PoH resume). **T5 split-brain (NetworkChaos between pods) and T6 multi-node chaos** are **not** faithful on single-node RD and get deferred to the cloud re-spin. | ✅ T0/T2-code/T3/T4 • ❌ T5/T6 chaos realism | ✅ closed 2026-04-25 (#80) — see Session Plan inline detail at line ~4408. T5/T6 deferred to cloud per design. |
| V3 | Processor validation — both native Rust and Wasm guests through per-event + batch paths; confirm L2/L3/WAL tiers engage per `DurabilityMode`; `outputs_acked_total == input` at steady state | ✅ yes | ✅ closed 2026-05-02 (#81) — Wasm/OrderedBatch 2026-04-25, Native rows N1+N2 2026-04-30, M2 checkpoints fix 2026-05-01, Wasm·per-event R2 + G4 outputs-acked 2026-05-02. See Session Plan inline detail at line ~4410. |
...
| V5 | Crypto chain E2E — walk a transferred partition under each `PohVerifyMode::{Verify, VerifyWithKey, TrustExtend}` and assert MMR + Merkle + Ed25519 root-sig round-trips, resumed `PohChain.sequence()` matches sender. All traffic is loopback but the crypto path is identical. | ✅ yes | ✅ closed 2026-04-30 (#83) — V5.1 manifest wiring 2026-04-29, R1 signing-key resolver + W1 cmd_serve install + W2 /poh-head signed-root surface + V1 independent verify-poh CLI all 2026-04-30, H1+H2 helm Secret wiring 2026-05-02. See Session Plan inline detail at line ~4411. |
| V6 | Consolidated `docs/GATE2-PRE-SESSION-B-VALIDATION.md` report + Session-B readiness checklist; back-propagate any new gaps to the Gate 2 blocker queue. Explicitly records the T5/T6-on-RD gap so the DOKS re-spin checklist carries it forward. | ✅ yes | ✅ closed 2026-04-24 (#84) — doc shipped 2026-04-24; rolled forward through 2026-05-02 with V2/V3/V5 closures and the W3 fault-injection investigation (commits cc58bb4 + b8c03c8 + 43c9ae6). |
...
| P4.iii | Pre-bake Aeon image to ECR (`us-east-1`) from the same feature-branch SHA used on DOKS. **Sequencing updated 2026-05-03** per [`STATEFUL-PROCESSING-EVOLUTION.md`](STATEFUL-PROCESSING-EVOLUTION.md) § 7 — Path 2 (G9.c, Layer 4 of the integrated stateful-processing trajectory) closes the cluster-correctness gap for the 7 vulnerable lifecycle endpoints AND lays the substrate for Layer 5+6 stateful additions; it ships in v0.1 BEFORE Session B so the EKS ceiling number reflects what operators would actually deploy. Order is now: G9.c (4–5 days) → ECR bake → 6-hour Session B → publish ceiling number → v0.1 cut. Stateful-processing Layers 5+6 open as the v0.2 broadening trajectory after v0.1. | Awaits G9.c close + user-driven Session B ECR bake (P4.i infrastructure shipped 2026-04-19; image bake itself is operator action, not a code gate) | ⏳ pending |
...
| P5.b | Per-sink dynamic checkpointing on transfer — revisit after Phase 3; may already be covered by existing `per_sink_ack_seq` plumbing | ✅ **closed 2026-05-03 as covered-by-existing-design**. Audit found that `CutoverOffsets` (passed at partition cutover) carries `final_source_offset` + `final_poh_sequence` but NOT `per_sink_ack_seq` — so a new partition owner starts with empty per-sink ack tracking. Two non-correctness effects: (1) L2 GC for transferred partition is delayed until new owner's sinks ack new events (self-healing in seconds); (2) if new owner crashes immediately after transfer before any events flow, recovery re-delivers from seq 0. **Both effects are absorbed by Aeon's existing EOS tier semantics**: T1 transactional DB / T2 transactional stream (Kafka idempotent producer) / T3 atomic rename file / T4 dedup-keyed / T5 idempotency-key all explicitly handle duplicate delivery; T6 fire-and-forget accepts duplicates by contract. Adding `per_sink_ack_seq` to `CutoverOffsets` is an optimization (faster L2 GC catch-up) that doesn't change correctness — worth a follow-up only if L2 storage pressure surfaces in production. Not blocking v0.1. |
...
| **B4** | **Consumer-group config mode for compatible pull sources** (Kafka / Redpanda / Redis Streams / Valkey Streams). Per-source `consumer_mode: { kind: single \| group, group_id, broker_commit }` where `single` is today's manual-assign (default, unchanged) and `group` uses the broker's subscribe-and-rebalance protocol. Broker auto-commit stays **off** in both modes — Aeon's EO-2 ledger remains the source of truth. Hard guardrail: `kind: group` is mutually exclusive with cluster-coordinated partition ownership, validated at pipeline start. Core pipeline (SPSC, Raft `partition_table`, EO-2) untouched. RabbitMQ super-streams explicitly out of scope (SAC incompatible with `reassign_partitions`). **Landed 2026-04-21**: new `aeon_types::ConsumerMode { Single, Group { group_id, broker_commit } }` enum + `Source::broker_coordinated_partitions()` default method (returns `false`). Kafka/Redpanda source branches on `consumer_mode` — `Single` keeps today's `assign()` path; `Group` uses `subscribe()`, overrides the rdkafka `group.id` with the mode's id, and wires `enable.auto.commit` from `broker_commit`. Kafka `reassign_partitions()` hard-errors in `Group` mode so a stray Raft watcher cannot stomp the broker's assignment. Redis Streams grows the same `consumer_mode` field + `effective_group()` helper that picks between the config's `group` and the mode's `group_id`. Pipeline-start guardrail in `run_buffered` / `run_buffered_transport` / `run_buffered_managed`: `partition_reassign.is_some() && source.broker_coordinated_partitions()` → config error at start. `MaybeL2Wrapped` / `L2WritingSource` / `DynSource` / `BoxedSourceAdapter` all forward `broker_coordinated_partitions` so the guard works through every pipeline wrapping layer. Tests: `ConsumerMode` enum (3), Kafka config (2), Redis config + `effective_group` override (2), pipeline guardrail happy + unhappy path (2). All green. | Not a bake blocker. Lands as its own mini-phase after B1/B2/B3. | ✅ |

**Ship order**: B1 → B2 (both required before bake), then B3 (polish,
same-session), then B4 (separate session post-bake).

**Status as of 2026-04-21**: B1 + B2 + B3 + B4 all shipped in the same
sweep. All pre-bake correctness blockers are closed and the deliberate
post-bake mini-phase B4 (consumer-group config mode) has also landed;
the only open bake-path item is P4.iii (task #6, pre-bake Aeon image to
ECR `us-east-1`).
...
### Full-Repo Audit — Security & Compliance Workstreams S1–S8 (2026-04-21)

A cross-cutting audit of roadmap + code + docs vs. OWASP Top 10, 12-factor
methodology, and Apache Flink feature parity surfaced items that the existing
pillar/phase structure didn't capture. These land as one sequenced set of
workstreams (S1–S8). Full detail in memory
`project_security_compliance_design.md`; connector-level fixes in
`docs/CONNECTOR-AUDIT.md` §9; this index tracks status only.

**Approved sequence** (2026-04-21):
...
| **S2**  | Tracing redaction layer + payload-never-in-logs sweep + build-time `debug-payload-logging` feature | **Closed 2026-04-21** |
...
- **S8.1** push-source `source_kind()` overrides on MQTT / RabbitMQ / QUIC /
  WebSocket / WebTransport (stream) / WebTransport (datagram) sources +
  `SourceKind::Poll` fix on `http/polling_source.rs`. Prevents silent
  data-loss under `durability != none` when a push source inherits the
  default `Pull` kind. See `docs/CONNECTOR-AUDIT.md` §9.1.
- **S8.2** sink `on_ack_callback()` overrides on NATS / Redis Streams / HTTP
  / MQTT / RabbitMQ sinks (Kafka was already wired). Drives
  `outputs_acked_total` so operators see the gap between enqueue and
  broker-confirmed delivery. §9.2.
- **S8.3** `aeon_types::L3Backend::RocksDb` variant gated behind new
  `rocksdb` cargo feature (re-exported from `aeon-state`). Without the
  feature, YAML configs selecting `rocksdb` fail at deserialization, not
  at runtime. §9.3.

**S2 — closed 2026-04-21**, three pieces:

- **S2.1** `aeon_types::redact::redact_uri` — zero-alloc `Cow<'_, str>`
  helper that strips `user[:password]@` from any URI-shaped string. Lives
  in `aeon-types` so every crate (connectors, engine, processor-client,
  CLI) can reach it without pulling in `aeon-observability`. 10 unit
  tests cover empty / no-scheme / no-userinfo / password-with-`@` /
  nested-scheme-in-query / fragment / unusual-scheme paths.
- **S2.2** Sweep of every connector + processor-client log site that
  previously emitted the raw connection URL (HTTP sink + polling source,
  WebSocket sink + source, WebTransport sink + processor-client
  WebTransport + processor-client WebSocket). Both `tracing::info!` /
  `tracing::warn!` sites and the `AeonError::connection(format!(...))`
  error chains are redacted — the audit finding C4 (Kafka SASL URL in
  logs) generalised to every URL-shaped connector field.
- **S2.3** `debug-payload-logging` cargo feature on `aeon-engine` with a
  `compile_error!` guard that refuses to compile in release builds
  (`#[cfg(all(feature = "debug-payload-logging", not(debug_assertions)))]`).
  The feature is a dev-only diagnostic; shipping it to prod would
  require changing the gate, not forgetting a CLI flag.

**S7 — closed 2026-04-22**, three pieces:

- **S7.1** `aeon_types::ssrf` module — `SsrfPolicy` with per-category
  toggles (`allow_loopback` / `allow_private` / `allow_link_local` /
  `allow_cgnat`) plus `extra_deny` / `extra_allow` CIDR lists
  (`ipnet::IpNet`). Three presets: `production()` (VPC-safe default —
  allows RFC1918, denies loopback / link-local / CGNAT / ULA),
  `strict()` (also denies private), `permissive_for_tests()` (allows
  everything EXCEPT the `ALWAYS_DENIED_HOSTS` set: AWS IMDS
  `169.254.169.254`, Alibaba `100.100.100.200`, GCP metadata
  `169.254.169.253`, Oracle `192.0.0.192` — layered over every preset
  so even a test config cannot accidentally dial the cloud metadata
  service). API: `check_addr(IpAddr)`, `check_host(host, port)` (full
  `ToSocketAddrs` resolve + filter), `check_url(&str)` (scheme-aware
  parse + default-port inference + userinfo strip + IPv6 literal
  support). `From<SsrfError> for AeonError` so call-sites just use
  `?`. 20 unit tests cover the full matrix — presets, IMDS layering,
  loopback v4 / v6, link-local v4 / v6, CGNAT 100.64.0.0/10, ULA
  fc00::/7, unspecified / multicast / broadcast, `extra_deny` and
  `extra_allow` precedence, `extra_allow` **cannot** override
  `ALWAYS_DENIED_HOSTS`, host/port parsing edge cases, JSON serde
  roundtrip defaults.
- **S7.2** Connector wiring at every URL-based dial site — HTTP sink,
  HTTP polling source, WebSocket sink, WebSocket source, WebTransport
  sink. Each connector gains a `ssrf_policy: SsrfPolicy` field (defaults
  to `production()`) plus a `with_ssrf_policy()` builder and runs
  `config.ssrf_policy.check_url(&config.url)?` at `::new()` before any
  socket opens. WebSocket source additionally re-runs the guard on
  every reconnect — anti-rebinding, so a DNS record that flips to a
  private IP between initial connect and now fails closed. Test
  fixtures in `crates/aeon-engine/tests/e2e_tier_e.rs` (3 sites) and
  `e2e_tier_f.rs` (2 sites) migrated to
  `aeon_types::SsrfPolicy::permissive_for_tests()` so loopback
  127.0.0.1 still works but IMDS stays blocked even in tests. 3 new
  connector-level SSRF rejection tests (HTTP sink IMDS, HTTP polling
  IMDS, WebSocket sink loopback-denied) written with
  `let Err(err) = ... else { panic!(...) }` because these builders
  don't impl `Debug` on the success variant.
- **S7.3** CLI YAML surface — `parse_ssrf_policy()` +
  `parse_cidr_list()` helpers on `aeon-cli/src/connectors.rs` translate
  six YAML keys (`ssrf_allow_loopback`, `ssrf_allow_private`,
  `ssrf_allow_link_local`, `ssrf_allow_cgnat`, `ssrf_extra_deny`,
  `ssrf_extra_allow`) into an `SsrfPolicy`. Wired into
  `HttpPollingSourceFactory` and `HttpSinkFactory` with updated docstrings
  listing the new keys. `extra_deny` / `extra_allow` accept
  comma-separated CIDR lists (e.g. `"10.0.0.0/8,192.168.0.0/16"`). 3 new
  parser tests cover defaults, explicit toggles, and CIDR-list parsing.
  See `docs/CONNECTOR-AUDIT.md` §9.7–§9.8.

**S1 — closed 2026-04-22**, three pieces:

- **S1.1** `aeon_types::secrets` module — `SecretScheme` enum
  (`Env` / `DotEnv` / `Vault` / `AwsSm` / `AwsKms` / `Literal`),
  `SecretRef { scheme, path }` with `parse()` that returns
  `Ok(None)` for plain strings and `Err` for malformed
  `${SCHEME:path}` tokens. `SecretBytes` wrapper — `zeroize` on
  drop, no `Debug` / no `Clone`, exposes `expose_bytes() -> &[u8]`
  and `expose_str() -> Result<&str, _>`. `SecretProvider` trait
  (`scheme() -> SecretScheme`, `resolve(&self, path) -> Result<SecretBytes, _>`)
  with three local implementations: `EnvProvider` (reads
  `std::env::var`), `DotEnvProvider` (reads a `.env` file whose
  path is supplied via `AEON_DOTENV_PATH`, **no auto-discovery**),
  `LiteralProvider` (returns raw value, `warn`-once per run so
  operators see they've inlined a secret). `SecretRegistry` owns
  a `HashMap<SecretScheme, Arc<dyn SecretProvider>>`;
  `default_local()` registers Env + DotEnv + Literal. The
  interpolator `interpolate_str<'a>(&self, s: &'a str) -> Result<Cow<'a, str>, _>`
  returns `Cow::Borrowed` when the input has no `$` (zero-alloc
  fast path), otherwise scans for `${SCHEME:path}` tokens,
  handles `${...}` escape, and errors on unknown scheme,
  unregistered provider, or malformed token. Vault / AWS SM / AWS
  KMS providers deliberately **not** implemented here — they'll
  live in a future `aeon-secrets` crate that downstream callers
  register via `SecretRegistry::register()`. `From<SecretError> for AeonError`
  routes to `AeonError::Config` (non-retryable). 34 unit tests
  cover the full matrix.
- **S1.2** `aeon_crypto::kek` module — dual-domain KEK
  separation: `KekDomain::{LogContext, DataContext}`. The two
  domains are **strictly non-fungible** — a DEK wrapped under
  `LogContext` cannot be unwrapped by a `DataContext` KekHandle,
  and vice versa (audit log keys and payload keys never cross,
  even if an operator misconfigures). `WrappedDek { kek_domain,
  kek_id, nonce, ciphertext }` is serde-serializable and wire-
  stable. `DekBytes([u8; 32])` — `zeroize` on drop, no `Debug`
  (redacted), no `Clone`. `KekHandle` holds `domain`, `active_id
  + active_ref`, optional `previous_id + previous_ref`, and an
  `Arc<SecretRegistry>`. `wrap_new_dek()` generates a random DEK
  and wraps it; `wrap_dek(&DekBytes)` wraps an existing one;
  `unwrap_dek(&WrappedDek)` dispatches on `wrapped.kek_id`
  (active vs previous) for **hot-key rotation**, rejecting
  domain mismatch and unknown kek_id. Envelope encryption is
  AES-256-GCM (12-byte random nonce, 128-bit tag) via the
  `aes-gcm` crate — same primitive planned for S3 at-rest
  encryption. 10 tests: active-roundtrip, previous-roundtrip
  (rotation), domain-mismatch rejection, unknown-kek_id
  rejection, tampered-ciphertext GCM-tag rejection, wrong-length
  KEK rejection, `DekBytes` debug-redacted, DEK generation
  non-zero, `KekDomain` string-stability, `WrappedDek` JSON
  serde roundtrip.
- **S1.3** CLI pre-parse YAML interpolation —
  `read_and_interpolate_manifest(&Path) -> Result<String>` helper
  on `aeon-cli/src/main.rs` reads the manifest, enforces the
  existing `MAX_MANIFEST_SIZE` limit, and runs
  `SecretRegistry::default_local().interpolate_str(&raw)` before
  handing the resolved string to `serde_yaml`. Wired into
  `cmd_apply` and `cmd_diff` (both need the resolved config).
  `cmd_check` deliberately left on raw-read — it's a syntax-only
  validator that operators should be able to run offline without
  env vars set. This one helper covers every connector: because
  interpolation happens pre-parse, no per-field plumbing is
  needed. Documented limitation: interpolated values containing
  YAML-reserved characters may break parsing (base64-encode
  multi-line secrets). 5 new tests cover env-ref replacement,
  plain-YAML passthrough, missing-env-var error, unregistered-
  scheme error, oversize-file rejection. Fixed 2 pre-existing
  SSRF test gaps in `connectors.rs`
  (`http_polling_source_requires_url`,
  `http_sink_factory_requires_url_and_posts`) that were using
  `127.0.0.1` against the default `SsrfPolicy::production()` —
  added `ssrf_allow_loopback: true` to both configs. Full CLI
  suite back to 34/34 green.

**S9 — closed 2026-04-22**, inbound authN/authZ on push-endpoint
sources where Aeon listens (HTTP ingest, WebTransport source, QUIC
source). Broker push sources (MQTT, RabbitMQ) authenticate via the
broker's own config and are **out of scope**. Stackable modes
declared in operator-chosen cheap-reject-first order:

- **IP allow-list** — CIDR list from env-var ref. Applied before any
  header parsing so unauthenticated bots can't even exercise the
  parser.
- **API-key header** — configurable header name (default
  `X-Aeon-Api-Key`); key material from the S1 secret provider.
  Multiple keys supported for rotation (active + prev-grace window).
- **HMAC-signed request** — shared secret from secret provider;
  signature header (default `X-Aeon-Signature`) =
  `hex(HMAC-SHA256(secret, method || "\n" || path || "\n" || ts || "\n" || body))`.
  Timestamp skew window configurable (default 300s); skew-window
  bounds replay without an explicit replay cache.
- **mTLS** — CA bundle validated by the TLS layer (rustls); verifier
  only checks subject (CN/SAN) allow-list membership.
- **JWT bearer** — **deferred to S9.2 post-Gate-2** (needs JWKS fetch
  + rotation infra).

Delivered across three sub-items:

- **S9.1** `aeon_types::auth` module — `hmac_sig` (sign/verify over
  canonical preimage, kebab-case `HmacAlgorithm`, constant-time
  compare via `hmac::Mac::verify_slice`, `[active, previous]`
  candidate list for rotation) + `inbound` (`InboundAuthMode` 4-way
  enum, per-block config structs, `InboundAuthConfig` top-level
  doc, `InboundAuthVerifier::build()` with up-front validation,
  `verify(&AuthContext)` running modes in declaration order,
  redacted custom `Debug`, `AuthRejection::reason_tag()` +
  `redacted_peer_ip()`). 48 new aeon-types tests (276 total,
  clippy clean all-targets).
- **S9.2** connector wiring. `HttpWebhookSource` gets the full
  4-mode surface: `ConnectInfo<SocketAddr>` for peer IP,
  `HeaderMap` → flat `&[(&str, &[u8])]`, auth runs before the
  push-buffer overload check, `status_for(rejection)` → HTTP
  401 (ApiKey / Hmac) vs 403 (IP / mTLS), `tracing::warn` with
  `reason_tag` + redacted peer IP. Raw push sources
  (`QuicSource`, `WebTransportSource`, `WebTransportDatagramSource`)
  receive IP allow-list at pre-handshake acceptance via
  `incoming.remote_address().ip()` — session refused
  (`quinn::Incoming::refuse()` / `wtransport::IncomingSession::refuse()`)
  before TLS work. API-key / HMAC / mTLS for QUIC+WT require
  header/cert-subject plumbing deferred to a follow-up. 8 new
  webhook auth tests covering all 4 modes with real reqwest
  round-trips. 40/40 connector tests green under
  `http,quic,webtransport`.
- **S9.3** CLI YAML surface. `parse_inbound_auth_config()` reads a
  flat-key convention matching the existing factory config shape
  (`auth_modes`, `auth_ip_cidrs`, `auth_api_keys`,
  `auth_hmac_secrets`, `auth_hmac_algorithm`,
  `auth_hmac_skew_seconds`, `auth_mtls_subjects` + header-name
  overrides). Absent `auth_modes` ⇒ no verifier installed. Secret
  fields carry plaintext by factory-time — S1.3 pre-parse
  interpolation resolves `${VAULT:...}` / `${ENV:...}` before YAML
  parse. `build_inbound_auth_verifier` wraps in `Arc` and plugs
  into `HttpWebhookSourceFactory::build()`. 5 new parser tests;
  full CLI suite 29/29 green under `rest-api`. QUIC + WT YAML
  wiring tracked under P5.c connector-catalog work.

Rejection surface: HTTP 401/403, WT/QUIC `refuse()` before TLS
handshake completes. Metric `aeon_inbound_auth_rejected_total{source,reason}`
driven off `AuthRejection::reason_tag()` (bounded-cardinality
...
- **`none`** — explicit, for pull sources inside a trusted VPC
  where the upstream (Kafka/Redpanda broker, PG/MySQL CDC replica,
  internal HTTP API behind an SG) authenticates Aeon by its
  **egress IP allow-list** configured on the upstream's side. This
  is the pragmatic default for broker / CDC pull sources —
  credential-less dial, network-layer auth. `audit-log-required:
  true` at pipeline start so operators see the deliberate choice.
  Deployment docs will cover K8s egress-IP pinning (NAT gateway /
  egress-IP pool) and VPC-peering recommendations so operators
  know which IPs to whitelist upstream.
- **`bearer`** — `Authorization: Bearer <ref>`.
- **`basic`** — user + password refs (PG, MySQL, HTTP-Basic).
- **`api_key`** — configurable header name + key ref.
- **`hmac_sign`** — same algo as S9 HMAC, reversed direction.
- **`mtls`** — client cert + key from secret provider.
- **`broker_native`** — delegates to the underlying client
  library's auth knobs (Kafka SASL/PLAIN, SASL/SCRAM, OAUTHBEARER;
  Redis AUTH/ACL; NATS creds). Credentials flow through S1 secret
  provider refs; the broker SDK receives resolved plaintext at
  connect time only.

All values resolved via the S1 secret provider — **never literal in
YAML**. Shared HMAC helper + header-parse primitives come from S9.

**S10 landing status (2026-04-22):**

- **S10.1 primitives closed** —
  `aeon-types::auth::outbound::{OutboundAuthMode, OutboundAuthConfig,
  OutboundAuthSigner, OutboundSignContext, OutboundAuthBuildError,
  OutboundSignError}` and per-mode config structs
  (`BearerConfig`, `BasicConfig`, `OutboundApiKeyConfig`,
  `HmacSignConfig`, `OutboundMtlsConfig`, `BrokerNativeConfig`).
  `OutboundAuthSigner::build()` moves secrets into `SecretBytes`
  (zeroize-on-drop); `http_headers()` emits per-request headers for
  Bearer / Basic / ApiKey / HmacSign and empty Vec for None / Mtls /
  BrokerNative (the latter two surface their material via
  `mtls_cert_pem()`/`mtls_key_pem()`/`broker_native()`). Typed
  internal `CompiledSigner` enum — no `.unwrap()` / `.expect()` on
  the hot path (FT-10). Custom redacted `Debug`. 20 tests, 296
  aeon-types tests total.
- **S10.2 HTTP wiring closed** — `HttpSinkConfig::with_auth()` and
  `HttpPollingSourceConfig::with_auth()` accept
  `Arc<OutboundAuthSigner>`. Per-request headers merged via
  `signer.http_headers(OutboundSignContext)` with method/path/body
  fed in. `Mtls` mode installs a `reqwest::Identity` on the client
  at build time (PEM cert+key concatenated, `Identity::from_pem`
  under rustls-tls). `BrokerNative` is logged-and-ignored for HTTP.
  Tests cover Bearer + HmacSign + ApiKey + None header injection
  end-to-end against an axum test server.
- **S10.3 CLI YAML surface closed** — `parse_outbound_auth_config`
  + `build_outbound_auth_signer` in `aeon-cli::connectors` read the
  flat-key shape: `auth_mode` (none/bearer/basic/api_key/hmac_sign/
  mtls/broker_native), `auth_bearer_token`, `auth_basic_username`,
  `auth_basic_password`, `auth_api_key_header` (default
  `X-Aeon-Api-Key`), `auth_api_key`, `auth_hmac_sign_signature_header`,
  `auth_hmac_sign_timestamp_header`, `auth_hmac_sign_secret`,
  `auth_hmac_sign_algorithm` (hmac-sha256/hmac-sha512),
  `auth_mtls_cert_pem`, `auth_mtls_key_pem`, and
  `auth_broker_native.<key>` for broker-native pass-through values.
  Wired into `HttpSinkFactory` + `HttpPollingSourceFactory`. 13 new
  parser tests, 42 aeon-cli connectors tests total (rest-api feat).
- **S10.4 WebSocket wiring closed (2026-04-22)** —
  `WebSocketSourceConfig::with_auth()` and
  `WebSocketSinkConfig::with_auth()` accept
  `Arc<OutboundAuthSigner>`. Headers (Bearer / Basic / ApiKey /
  HmacSign) are injected on the WebSocket upgrade handshake via
  `IntoClientRequest` → `http::Request<()>` before
  `tokio_tungstenite::connect_async`. HMAC canonicalizes
  `method="GET" | path=<parsed> | ts | body=""`. Reconnect loop in
  the source re-signs on every reconnect (HMAC timestamps stay
  fresh) and re-runs the SSRF guard against anti-rebinding. `Mtls`
  mode is logged-and-ignored at this layer (requires a custom
  rustls connector on tungstenite — tracked as follow-up);
  `BrokerNative` is warned-and-ignored (not applicable). 10 unit
  tests on `build_ws_request` + `extract_path`, no live-server
  dependency.
- **S10.5 Kafka broker-native wiring closed (2026-04-22)** —
  `KafkaSourceConfig::with_auth()` and
  `KafkaSinkConfig::with_auth()` accept `Arc<OutboundAuthSigner>`.
  New `aeon-connectors/src/kafka/auth.rs` module translates modes
  onto rdkafka's `ClientConfig`: `BrokerNative` iterates
  `signer.broker_native().values` and passes SASL / SSL keys
  (`security.protocol`, `sasl.mechanism`, `sasl.username`,
  `sasl.password`, `sasl.oauthbearer.config`, `ssl.ca.location`,
  …) verbatim; `Mtls` sets `security.protocol=ssl` +
  `ssl.certificate.pem` + `ssl.key.pem` inline PEM (librdkafka ≥
  1.5); HTTP-style modes (Bearer / Basic / ApiKey / HmacSign) warn
  and skip. Signer knobs are applied AFTER the base rdkafka
  defaults but BEFORE `config_overrides`, so the existing
  debug/override escape hatch still wins. Wired into
  `KafkaSourceFactory::build()` + `KafkaSinkFactory::build()` via
  the same `build_outbound_auth_signer` plumbing that already
  covers HTTP. 5 new unit tests on the translator (`none` /
  `broker_native` / `mtls` / `bearer` warn-skip / absent-signer).
- **S10.6 Redis broker-native wiring closed (2026-04-22)** —
  `RedisSourceConfig::with_auth()` and
  `RedisSinkConfig::with_auth()` accept `Arc<OutboundAuthSigner>`.
  New `aeon-connectors/src/redis_streams/auth.rs` module translates
  the signer into a `redis::ConnectionInfo` before
  `redis::Client::open`: `BrokerNative` parses the URL via
  `IntoConnectionInfo` and then overlays `username` / `password` /
  `db` from `signer.broker_native().values` onto
  `ConnectionInfo.redis`, so the signer wins over any URL-embedded
  userinfo (a warn line records the conflict). `Mtls` is
  warn-and-skip — the workspace's `redis = "0.27"` dep does not
  enable the `tls-rustls` feature, so client certs can't be wired
  (tracked as follow-up; `rediss://` URLs still give server-auth
  TLS via `BrokerNative`). HTTP-style modes warn and skip. 7 unit
  tests covering every mode branch plus bad-URL / non-integer-db
  error paths. aeon-cli factory wiring for Redis source/sink is
  deferred to P5.c (same registry bucket as WebSocket).
- **S10.7 NATS broker-native wiring closed (2026-04-22)** —
  `NatsSourceConfig::with_auth()` and
  `NatsSinkConfig::with_auth()` accept `Arc<OutboundAuthSigner>`.
  New `aeon-connectors/src/nats/auth.rs` module exposes
  `connect_with_auth(url, signer)` which builds an
  `async_nats::ConnectOptions` then calls `.connect(url)`. For
  `BrokerNative`, the translator picks one of `credentials` (inline
  `.creds` JWT+NKey), `nkey_seed`, or `token` in that order of
  precedence, and independently applies `username` + `password`
  when both keys are present (either alone → config error so
  misconfig is loud, not silently under-authenticated). `Mtls` is
  warn-and-skip: async-nats 0.38 `add_client_certificate` takes
  `PathBuf` not inline PEM, and writing secret material to tmp
  files would violate the S1 "no secrets on disk outside deploy
  dir" posture — a follow-up can either patch async-nats for
  in-memory PEM or use a secure ramfs. HTTP-style modes warn and
  skip. 10 unit tests in `nats/auth.rs` covering every branch plus
  malformed-credentials and missing-username-or-password errors.
  aeon-cli factory wiring for NATS is deferred to P5.c.
- **S10.8 CDC broker-native wiring closed (2026-04-22)** —
  Postgres / MySQL / MongoDB CDC source configs all gain
  `with_auth(Arc<OutboundAuthSigner>)`. Each connector now has a
  dedicated `auth.rs` translator:
  - `postgres_cdc/auth.rs` parses the connection string into a
    `tokio_postgres::Config`, then overlays `user` / `password` /
    `dbname` / `application_name` from `broker_native().values`.
    `Mtls` is warn-and-skip — wiring requires the
    `tokio-postgres-rustls` crate, not in the workspace. 8 unit
    tests.
  - `mysql_cdc/auth.rs` parses the URL into `mysql_async::Opts`,
    builds via `OptsBuilder::from_opts`, overlays `user` /
    `password` / `db_name`. `Mtls` is warn-and-skip — mysql_async
    `SslOpts` requires the `native-tls-tls` or `rustls-tls` cargo
    feature, not enabled in this build. 7 unit tests.
  - `mongodb_cdc/auth.rs` parses the URI into
    `mongodb::options::ClientOptions` (async), then overlays
    `Credential.username` / `.password` / `.source` from
    `broker_native().values.{username,password,auth_source}`.
    `Mtls` is warn-and-skip — `TlsOptions::cert_key_file_path`
    takes a `PathBuf`, inline PEM would violate S1 "no secrets on
    disk outside deploy dir". 8 unit tests (`#[tokio::test]`
    because `ClientOptions::parse` is async).

  All three translators apply the "signer wins over URI userinfo"
  rule with a warn line on conflict, matching Kafka / Redis / NATS.
  HTTP-style modes warn-and-skip on all three. aeon-cli factory
  wiring for the CDC sources is deferred to P5.c.
- **S10.9 WebTransport sink HTTP/3 CONNECT header auth closed
  (2026-04-22)** — `WebTransportSinkConfig::with_auth()` accepts
  `Arc<OutboundAuthSigner>`. HTTP-style modes (Bearer / Basic /
...
```yaml
compliance:
  regime: pci | hipaa | gdpr | mixed | none   # default: none
  enforcement: strict | warn | off            # default: off
  selectors:                                  # optional PII/PHI field list
    - path: "$.user.ssn"
      format: json            # json | message_pack | binary_length_prefix
      class: pii              # pii | phi
```

`strict` is a hard gate at pipeline start: the precondition validator
refuses to bring the pipeline up if the environment does not satisfy
the regime's preconditions (S3 at-rest encryption enabled; S1 Vault /
KMS secrets in use; S5 retention schedule configured; S6 erasure API
wired for GDPR / mixed). `warn` logs the same findings but starts;
`off` is the development default. PII/PHI selectors are declarative
field descriptors consumed by the at-rest encryption path (S3),
retention (S5), and erasure (S6) — they are not evaluated in the
compliance module itself. **Protobuf is intentionally out of scope**
...
`${AWS_KMS:key}`, `${DOTENV:VAR}` interpolation resolved by the S1
secret-provider trait at pipeline start. Helm values map 1:1 to env
var names (documented in `docs/DEPLOYMENT.md`). Literal values in
YAML emit a load-time warning — dev-only. `.env` fallback is a file
outside the deployed artifact / container image, 0600 perms, path
supplied via `AEON_DOTENV_PATH`; no auto-discovery.

S1, S3–S6 workstreams are tracked against the design decisions locked in memory
...
| 5     | **V3** Processor validation       | ✅ closed 2026-04-25; native row landed 2026-04-30 | Wasm passthrough + `DurabilityMode::OrderedBatch` validated end-to-end on `aeon:b0d0d41`. Per pod: 500K events received / processed / sent, zero failures, `checkpoints_written_total=1`, **L2 segment 177,372,652 bytes (size-equal) across all 3 pods** at `/app/artifacts/l2body/v3-wasm-ordered/p00000/00000000000000000000.l2b`. Closure required three sequential bug-fixes — see "Live cluster validation tranche" below. **Native row closed 2026-04-30 (N1, image `aeon:v54`):** new cdylib companion crate `samples/processors/rust-native-cdylib/` exports the existing `JsonEnrichProcessor` via `aeon_native_sdk::export_processor!`; Dockerfile bundles it at `/app/processors/rust-native-v1.so`; supervisor's `build_processor` now consults `processor.tier` (carried through `ProcessorRef.tier` after a manifest-bridge update) and routes `tier: native` to `NativeProcessor::load`. Live cluster proof: applied `pipeline-v3-native-ordered.yaml`, supervisor logged `loaded native processor pipeline=v3-native-ordered processor=rust-native-v1 artifact=/app/processors/rust-native-v1.so`, 100K events received / processed / sent on each pod, 0 failures, L2 segment 35,368,556 bytes per pod (size-equal across all 3 pods; SHA256 differs because the memory source uses `identity: random` — same caveat applies to the prior v3-wasm-ordered "byte-identical" claim, which is actually size-equal). **Per-event row closed 2026-04-30 (N2):** new `pipeline-v3-native-perevent.yaml` with `durability.mode: per_event` + `count: 10000` + native processor; live run on `aeon:v54` showed 10K events received / processed / sent on each pod, 0 failures, L2 segment 3,529,628 bytes — confirms PerEvent runs cleanly under fsync amplification at modest counts. **`checkpoints_written_total=0` divergence resolved 2026-05-01 (M1+M2, image `aeon:v57`):** root cause was three layered gaps — (a) supervisor never installed an L3 store, so `CheckpointBackend::StateStore` silently no-op'd; (b) `pipeline_config_for` ignored the manifest's `checkpoint.backend` field, defaulting everything to `Wal`; (c) `run_sink_task` only called `write_checkpoint` for non-blocking strategies, so OrderedBatch / PerEvent never ticked the metric until shutdown. Fix: `aeon-cli::cmd_serve` now opens `${artifact_dir}/l3-checkpoints.redb` and installs it on `PipelineSupervisor::set_l3_checkpoint_store`; `pipeline_config_for` translates `CheckpointBackendDecl` → `CheckpointBackend`; periodic `write_checkpoint` added to the blocking-strategy path. Live proof on `aeon:v57`: `state_store` manifest declares trigger `Checkpoint L3 store initialized` log; OrderedBatch run reports `checkpoints_written_total=2`, PerEvent reports `=30`. **WAL-fallback live trigger (C1+C2)** Chaos Mesh installed via helm in namespace `chaos-mesh`; new `docs/examples/chaos-l3-fault-eaccess.yaml` injects EACCES on the L3 redb file via `toda` FUSE proxy; lifecycle discipline (apply chaos → run pipeline → stop pipeline → delete chaos) documented inline to avoid `ENOTCONN` mount-tear-down on open file handles. Behavioral evidence: chaos-mesh status reports `AllInjected: True` against the leader pod; engage_fallback's `tracing::warn!()` fires when an L3 write hits EACCES (covered by 5 existing `eo2_recovery` unit tests). Pre-existing FT-1/FT-2 caveat: cluster bootstrap still uses `MemLogStore` (Raft state non-persistent), so force-deleting pods can leave orphaned redb file locks — clean restart procedure documented. WAL→L3 drain on chaos recovery covered by `try_recover_primary` unit test, not yet live-triggered. **O1 closed 2026-05-01 (image `aeon:v58`):** spliced the supervisor's shared `Eo2Metrics` registry into the REST `/metrics` handler — `aeon_l2_bytes`, `aeon_l2_segments`, `aeon_l2_gc_lag_seq`, `aeon_l2_pressure{level=...}`, `aeon_sink_ack_seq`, and `aeon_checkpoint_fallback_wal_total` are now scrape-visible. Pre-O1 the in-process counters ticked but were invisible to Prometheus — `engage_fallback`'s increment was only observable via unit tests. Live-verified on `aeon:v58`: 500K-event run shows `aeon_l2_pressure{level=node|partition|pipeline} 0` and `aeon_checkpoint_fallback_wal_total 0` alongside the pre-existing per-pipeline counters. Counter stays at 0 under chaos for the same toda-ENOTCONN reason documented in commit `cc58bb4`; metric exposure itself is now correct. **P1 + Q1 closed 2026-05-01 (image `aeon:v60`):** P1 added an idle-path periodic `write_checkpoint` for blocking strategies (defensive correctness — fires when source drains and idles past the interval). Q1 propagates the manifest's `durability.checkpoint.interval` string onto `delivery.flush.interval` at runtime — pre-Q1 the field was parsed and round-tripped through the registry but never reached the engine, so blocking pipelines stuck at the 1-second default cadence. Live proof: same `l3-grow-validate` fixture (500K events, mode=per_event, manifest `checkpoint.interval: 50ms`) now reports `checkpoints_written_total=303` (was 2 on v59) — ~20 ticks/sec × ~15s run matches the 50ms interval exactly. L3 redb file size stays at 1,589,248 bytes because redb pre-allocates pages on open; record survival across pipeline restart is the authoritative growth proof, covered by the existing `checkpoint_statestore_persists_across_pipeline_restart` integration test. **FT-1 + FT-2 closed 2026-05-02 (image `aeon:v61`):** the pre-existing FT-1/FT-2 caveat (cluster bootstrap uses `MemLogStore`, Raft state non-persistent, force-delete leaves orphan redb locks) is resolved. New `ClusterNode::bootstrap_multi_persistent` and `::join_persistent` open `${artifact_dir}/raft.redb` (separate from `l3-checkpoints.redb` to keep concerns clean) and back the Raft log + state machine via `L3RaftLogStore::open` + `StateMachineStore::new_persistent`. `cmd_serve` selects the persistent variant when the redb open succeeds and falls back to the in-memory variant with a warn log on failure. `raft.initialize` recognises `InitializeError::NotAllowed` as the recovery-from-persisted-log path and continues silently. Live cluster validation: created pipeline `l3-grow-validate`, killed the leader (aeon-0, node 1, term=8, last_applied=4); cluster recovered to leader=node 2 (aeon-1) at term=9 (+1 increment, NOT term=1 reset) with last_applied=5 (+1 for the new-leader's partition-table update); pipeline `l3-grow-validate` survived in the recovered state machine. Pre-FT-1+FT-2, the same test split-brains the cluster at term=1 with empty pipeline list. 172/172 cluster lib tests green. **H1 + H2 closed 2026-05-02 (helm chart wiring for `poh.signingKey`):** new `cluster.poh.signingKey` block in `helm/aeon/values.yaml` supports three delivery shapes — `valueFromSecretKeyRef` (production, k8s Secret), `value` (literal, dev only, discouraged), or unset (env var omitted entirely). `helm/aeon/templates/statefulset.yaml` renders the env block via `{{- with .Values.cluster.poh }}{{- if and .signingKey .signingKey.envVar }}` guard so legacy installs that don't set `signingKey` are byte-identical. Live cluster proof on `aeon:v61` + helm revision 13: `kubectl create secret generic aeon-poh --from-literal=signing-key-hex=22..22`, `helm upgrade aeon ./helm/aeon -f ./helm/aeon/values-local.yaml -f /tmp/values-poh-overlay.yaml`, env spec on `aeon-0` confirmed to use `valueFrom.secretKeyRef` (NOT inline value), `printenv AEON_TEST_POH_SIG` on all 3 pods returns the Secret content. Full V5.1 walk against this Secret-driven config was already proven on `aeon:v53` (commit `eff52d6`) — H2 just adds the production-shape chart wiring. **P2 closed 2026-05-02 (idle-path P1 cluster proof):** new `docs/examples/pipeline-p1-idle-path.yaml` (count=256, batch_size=256, partitions=1, mode=per_event, checkpoint.interval=200ms) drained one batch in ~1 s and `aeon_pipeline_checkpoints_written_total{pipeline="p1-idle-path"} = 1` was observable on the leader's /metrics for the entire subsequent 30 s idle period. Pre-P1 (commit `d879fb1`), this fixture would have shown `=0` for the whole idle period and only ticked to 1 at pipeline shutdown via the final-flush path — observable mid-run only with the P1 fix. Subsequent ticks don't fire because `delivered_since_checkpoint` resets to 0 after the first idle-path write, exactly as the P1 design intended. **G4 closed 2026-05-02 (outputs-acked metric exercise):** new `docs/examples/pipeline-g4-kafka-acked.yaml` routes 10 K events through Memory → __identity → Kafka sink targeting Redpanda. `outputs_sent_total` and `outputs_acked_total` both reach 10 000 — the latter was wired in Phase 1 P1.1e–P1.1g but stayed at 0 across every cluster fixture because they all used Blackhole sinks. Kafka sink's `Sink::on_ack_callback` (crates/aeon-connectors/src/kafka/sink.rs:476) fires per broker-acked batch, the engine increments the metric, /metrics surfaces it. **W3 (live WAL-fallback chaos trigger) marked covered-but-not-live-injectable 2026-05-02:** the fault → fallback → metric path is fully covered by `eo2_recovery::tests::fallback_transition_bumps_metrics_counter` (unit) plus the O1 live `/metrics` exposure (commit `8c8dbb5`); bridging them with a single chaos-driven observation under k3s + local-path + redb with mmap'd open fds is blocked because chmod / truncate / rm / IOChaos-delay don't propagate to the open fd as a typed write error. Documented chaos-tooling investigation inline in `docs/examples/chaos-l3-fault-eaccess.yaml`. **F1 + F2 + F3 closed 2026-05-02 (image `aeon:v63`) — single-observation WAL-fallback proof via debug fault injection:** new `aeon-engine::debug_fault::FaultyL3Wrapper` wraps the supervisor's L3 store with an atomic "fail next N writes" counter (5 unit tests). New REST `POST /api/v1/test/inject-l3-fault?count=N` arms the wrapper. New `aeon inject-l3-fault --count N --api <url>` CLI subcommand drives it. `cmd_serve` always installs the wrapper around the redb store (no-op when fault counter is 0, so production-safe). Critical wiring fix: `build_sink_task_ctx`'s `FallbackCheckpointStore::new(...)` now chains `.with_metrics(eo2_metrics)` so engagement actually bumps the counter — pre-fix the store was built without metrics so engage_fallback ran but the counter stayed at 0 (discovered during F3 v62 run). Live cluster proof on `aeon:v63`: started `f3-fault` pipeline (10K events, partitions=1 on aeon-0), injected 100 faults via CLI, polled `/metrics` for 10 seconds — `aeon_checkpoint_fallback_wal_total` ticked from `0 → 1` within the first poll and stayed at 1 (correct: counter tracks transitions, not per-write faults; once engaged, subsequent faulted writes route to WAL without re-incrementing). The 51 of 100 armed faults that fired during the run successfully diverted L3 writes to the WAL sidecar without breaking the pipeline. **Q1 closed 2026-05-02 (helm chart wiring for tmpfs-backed artifacts mount):** new `persistence.artifactsTmpfs.enabled` + `sizeLimit` values in `helm/aeon/values.yaml` + matching `{{- if not .Values.persistence.artifactsTmpfs.enabled }}` guard on the artifacts volumeClaimTemplate in statefulset.yaml. When enabled, swaps the PVC for an `emptyDir.medium: Memory` of bounded size — operator can `dd` a filler to consume the space and trigger real ENOSPC into the L3 redb store (vs the F1+F2+F3 synthetic injection). Inline test recipe documented in values.yaml. Helm template smoke verified both shapes render correctly. NOT for production (tmpfs wipes on pod restart); the F1 synthetic injection remains the cleaner test path. |
