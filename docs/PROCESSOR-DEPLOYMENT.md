...
| ~~ZD-5~~ | ~~Blue-green runtime wiring~~ | `aeon-engine/src/pipeline.rs`, `rest_api.rs` | **Done (2026-04-11)** — `PipelineControl.start_blue_green()` installs green processor via `UpgradeAction` slot; processor task picks it up (green installed, blue remains active). `cutover_blue_green()` swaps green→active. `rollback_upgrade()` drops green. REST `/cutover` and `/rollback` call PipelineControl when handle exists. REST `POST .../upgrade/blue-green` now loads artifact → `instantiate_processor()` → `ctrl.start_blue_green()`. 2 tests: cutover + rollback. |
| ~~ZD-6~~ | ~~Canary traffic splitting~~ | `aeon-engine/src/pipeline.rs`, `rest_api.rs` | **Done (2026-04-11)** — `PipelineControl.start_canary(proc, pct)` installs canary processor; processor task splits events by `event.id % 100 < pct` (deterministic). `set_canary_pct()` adjusts percentage via `AtomicU8`. `complete_canary()` promotes canary to sole processor. REST `POST .../upgrade/canary` now loads artifact → `instantiate_processor()` → `ctrl.start_canary()`. `POST .../promote` calls `ctrl.set_canary_pct()` or `ctrl.complete_canary()`. 2 tests: canary-split + canary-complete. |

### 13.3 Source/Sink Zero-Downtime Reconfiguration

| # | Item | Approach | Details |
|---|------|----------|---------|
| ZD-7 | Same-type source config change | Drain→swap→resume | **Done (2026-04-11)** — `PipelineControl.drain_and_swap_source()`: pause source → drain rings → downcast `Box<dyn Any>` to concrete `S` → swap → resume. Test: `managed_pipeline_source_swap`. REST: `POST .../reconfigure/source` with same-type enforcement + `PipelineManager::reconfigure_source()`. 4 REST API tests + 3 unit tests. |
| ZD-8 | Same-type sink config change | Drain→swap→resume | **Done (2026-04-11)** — `PipelineControl.drain_and_swap_sink()`: pause source → drain rings → downcast `Box<dyn Any>` to concrete `K` → swap in `run_sink_task` → resume. Test: `managed_pipeline_sink_swap`. REST: `POST .../reconfigure/sink` with same-type enforcement + `PipelineManager::reconfigure_sink()`. |
