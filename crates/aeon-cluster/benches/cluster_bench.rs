...
async fn bench_single_node_bootstrap() -> (Duration, Duration) {
    ...
    let node = ClusterNode::bootstrap_single(ClusterConfig::single_node(1, 16)).await.unwrap();
    let bootstrap_time = start.elapsed();

    // Measure propose latency (single commit on single-node — instant commit)
    let start = Instant::now();
    let _resp = node.propose(ClusterRequest::AssignPartition {
...
async fn bench_single_node_throughput(n: usize) -> (Duration, f64) {
    ...
    let node = ClusterNode::bootstrap_single(ClusterConfig::single_node(1, 256)).await.unwrap();
...
impl ThreeNodeCluster {
    async fn create() -> Self {
        ...
        let ep1 = Arc::new(QuicEndpoint::bind("127.0.0.1:0".parse().unwrap(), server_cfg.clone(), client_cfg.clone()).unwrap());
        let ep2 = Arc::new(QuicEndpoint::bind("127.0.0.1:0".parse().unwrap(), server_cfg.clone(), client_cfg.clone()).unwrap());
        let ep3 = Arc::new(QuicEndpoint::bind("127.0.0.1:0".parse().unwrap(), server_cfg, client_cfg).unwrap());
...
async fn main() {
    ...
    println!("  Bootstrap (16 partitions):     {:>10.3} ms", bootstrap.as_secs_f64() * 1000.0);
    println!("  Single propose latency:        {:>10.3} ms", propose_latency.as_secs_f64() * 1000.0);

    for &n in &[100, 1000] {
        let (elapsed, throughput) = bench_single_node_throughput(n).await;
        println!("  {} proposals:  {:>10.3} ms  ({:.0} proposals/sec)", n, elapsed.as_secs_f64() * 1000.0, throughput);
    }
    ...
    println!("  Cluster formation:             {:>10.3} ms", formation_time.as_secs_f64() * 1000.0);

    let commit_latency = bench_three_node_commit_latency().await;
    println!("  Single commit latency:         {:>10.3} ms", commit_latency.as_secs_f64() * 1000.0);
    ...
    for &n in &[10, 50] {
        ...
        println!("  {} entries: propose {:>8.3} ms, converge {:>8.3} ms",
            n, propose_time.as_secs_f64() * 1000.0, converge_time.as_secs_f64() * 1000.0);
    }
    ...
    for &(partitions, nodes) in &[(16, 3), (256, 5), (1024, 10)] {
        ...
        println!("  {} partitions / {} nodes: {:>10.3} µs",
            partitions, nodes, elapsed.as_secs_f64() * 1_000_000.0);
    }
